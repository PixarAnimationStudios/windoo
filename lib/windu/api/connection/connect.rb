  ...
  module API
    ...
    class Connection
      ...
      module Connect
        ...
        # either:
        #
        # A Windu::API::Connection::Token object, which can be extracted from an active
        # Windu::API::Connection via its #token method
        #
        # or
        #
        ...
        # ### Default values
        #
        # Any values available via Windu.config will be used if they are not provided
        # in the parameters. See Windu::Configuration. If there are no config values
        ...
        #   if not defined in Windu.config
        #
        ...
        def validate_connected
          using_dft = 'Windu::API.cnx' if self == Windu::API.cnx
          unless connected?
        ...
        def parse_token(params)
          ...
          @token = Windu::API::Connection::Token.new(
        ...
        def apply_defaults_from_config(params)
          ...
          params[:host] ||= Windu.config.title_editor_server_name
          params[:port] ||= Windu.config.title_editor_server_port
          params[:user] ||= Windu.config.title_editor_username
          params[:timeout] ||= Windu.config.title_editor_timeout
          params[:open_timeout] ||= Windu.config.title_editor_open_timeout
          params[:ssl_version] ||= Windu.config.title_editor_ssl_version

          # if verify cert was not in the params, get it from the prefs.
          # We can't use ||= because the desired value might be 'false'
          params[:verify_cert] = Windu.config.title_editor_verify_cert if params[:verify_cert].nil?
        ...
        def verify_basic_params(params)
          ...
          raise Windu::MissingDataError, 'No :host specified in params or configuration.' unless params[:host]
          ...
          raise Windu::MissingDataError, "No :pw specified for user '#{params[:user]}'" unless params[:pw]
        end
