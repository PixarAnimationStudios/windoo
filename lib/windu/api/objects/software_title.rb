...
module Windu
  ...
  module API
    ...
    class SoftwareTitle < Windu::API::BaseClasses::JSONObject
      ...
      ####
      def self.all
        Windu::API.cnx.get(self::RSRC_PATH)
      end

      # @return [Windu::API::SoftwareTitle]
      ####
      def self.fetch(ident = nil, **key_and_ident)
        ...
        new Windu::API.cnx.get("#{self::RSRC_PATH}/#{id}")
      end
      ...
      def self.autofill_patches(ident)
        ...
        Windu::API.cnx.get("#{self::RSRC_PATH}/#{id}/patches/autofill")
      end
      ...
      def self.autofill_requirements(ident)
        ...
        Windu::API.cnx.get("#{self::RSRC_PATH}/#{id}/requirements/autofill")
      end
      ...
      JSON_ATTRIBUTES = {
        ...
        extensionAttributes: {
          class: Windu::API::ExtensionAttribute,
          multi: true
        }
      }.freeze
      ...
      def initialize(_json_data)
        super
        @requirements = requirements.map { |data| Windu::API::Requirement.new data }
        @patches = patches.map { |data| Windu::API::Patch.new data }
        @extensionAttributes = extensionAttributes.map do |data|
          Windu::API::ExtensionAttribute.new data
        end
      end
